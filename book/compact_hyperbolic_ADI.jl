        ...
        for k in 2:N[p]
            for j in 1:M[p]-1  # 固定j
                ...
                for i in 1:M[p]-1
                    ...
                    numerical_right_vector[i] = (1/144)*(Numerical[i,j,k]+10*Numerical[i+1,j,k]+
                        Numerical[i+2,j,k]+10*(Numerical[i,j+1,k]+10*Numerical[i+1,j+1,k]+Numerical[i+2,j+1,k])+
                        Numerical[i,j+2,k]+10*Numerical[i+1,j+2,k]+Numerical[i+2,j+2,k])+
                        (1/144)*(τ^2/2)*(f_func(x[i],y[j],t[k])+10*f_func(x[i+1],y[j],t[k])+f_func(x[i+2],y[j],t[k])+
                        10*(f_func(x[i],y[j+1],t[k])+10*f_func(x[i+1],y[j+1],t[k])+f_func(x[i+2],y[j+1],t[k]))+
                        f_func(x[i],y[j+2],t[k])+10*f_func(x[i+1],y[j+2],t[k])+f_func(x[i+2],y[j+2],t[k]))
                end
